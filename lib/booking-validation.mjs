...
const istParts=iso=>{const d=parseDateTime(iso),parts=new Intl.DateTimeFormat('en-GB',{timeZone:'Asia/Kolkata',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',hourCycle:'h23'}).formatToParts(d),out={};for(const p of parts)if(p.type!=='literal')out[p.type]=Number(p.value);return out};
const istDayOrdinal=iso=>{const p=istParts(iso);return Math.floor(Date.UTC(p.year,p.month-1,p.day)/86400000)};
const nightKey=iso=>{const p=istParts(iso);if(!(p.hour>=22||p.hour<5))return null;let stamp=Date.UTC(p.year,p.month-1,p.day);if(p.hour<5)stamp-=86400000;return new Date(stamp).toISOString().slice(0,10)};
...
async function computeOutstationRoute(pickupPlaceId,stops,finalDropPlaceId){
 ...
 const response=await fetch('https://routes.googleapis.com/directions/v2:computeRoutes',{method:'POST',headers:{'Content-Type':'application/json','X-Goog-Api-Key':key,'X-Goog-FieldMask':'routes.distanceMeters,routes.duration,routes.legs.distanceMeters,routes.legs.duration'},body:JSON.stringify({origin:{address:OUTSTATION_BASE_ADDRESS},destination:{address:OUTSTATION_BASE_ADDRESS},intermediates:[{placeId:pickup},...stopIds.map(placeId=>({placeId})),{placeId:finalDrop}],travelMode:'DRIVE',routingPreference:'TRAFFIC_UNAWARE',computeAlternativeRoutes:false,languageCode:'en-US',units:'METRIC'})});
 ...
 const route=payload.routes?.[0],meters=Number(route?.distanceMeters);if(!Number.isFinite(meters)||meters<=0)fail('Unable to calculate driving distance for the selected outstation route.');
 return {distanceMeters:meters,distanceKmExact:Math.round(meters/100)/10,billableRouteKm:Math.ceil(meters/1000),duration:String(route.duration||''),legs:(route.legs||[]).map(l=>({distanceMeters:Number(l.distanceMeters)||0,duration:String(l.duration||'')}))};
}
