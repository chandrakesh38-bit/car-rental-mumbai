...
export async function validateBookingData(serviceMode,data){
 ...
 if(serviceMode==='selfdrive'){
  ...
  if(mode==='home'){deliveryLocation=text(data.deliveryLocation,150);const km=METRO[deliveryLocation.toLowerCase()];if(!Number.isFinite(km))fail('Sorry, this delivery location is currently outside our standard service zones. Please select from our listed Mumbai metro locations or contact support for custom outstation/delivery quotes.');
 ...
 else {const ret=parseDateTime(data.returnAt);if(ret<=pickup)fail('Return date and time must be later than pickup date and time.');const destination=text(data.destination,150);const distance=DESTINATIONS[destination.toLowerCase()]||300;const days=Math.max(1,Math.ceil((new Date(ret.toDateString())-new Date(pickup.toDateString()))/86400000)+1);const billable=Math.max(distance,days*rules.minimumOutstationKmPerDay);fare=billable*number(car.outstation_rate_per_km)+days*number(car.driver_allowance_per_day)+allowance;route=text(data.pickupLocation,300)+' → '+destination;data={...data,days,billableKm:billable};}
