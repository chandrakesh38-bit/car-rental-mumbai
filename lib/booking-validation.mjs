...
async function computeOutstationRoute(pickupPlaceId,stops,finalDropPlaceId,journeyType){
 if(!['one-way','round-trip'].includes(journeyType))fail('Choose one-way or round trip.');
 ...
 const returnIntermediates=journeyType==='round-trip'?[{placeId:pickup}]:[];
 const second=await googleRouteRequest({origin:{placeId:finalDrop},destination:{address:OUTSTATION_BASE_ADDRESS},...(returnIntermediates.length?{intermediates:returnIntermediates}:{}),travelMode:'DRIVE',routingPreference:'TRAFFIC_UNAWARE',computeAlternativeRoutes:false,languageCode:'en-US',units:'METRIC'});
 ...
 return {distanceMeters:meters,distanceKmExact:Math.round(meters/100)/10,routeKmRoundedUp:Math.ceil(meters/1000),journeyType,legs};
}
...
export async function validateBookingData(serviceMode,data){
 ...
 if(tripType==='local'){await validatePickupServiceArea(data.pickupPlaceId);if(!PACKAGES.has(data.localPackage))fail('Invalid local package.');const base=number(car.local_pkg_8hr_80km),extra=number(car.local_extra_hour_rate);const extraHours={ '8hr_80km':0,'10hr_100km':2,'12hr_120km':4}[data.localPackage];const packageHours={ '8hr_80km':8,'10hr_100km':10,'12hr_120km':12}[data.localPackage];const end=new Date(pickup.getTime()+packageHours*3600000).toISOString();const nightCharge=nightCount([data.pickupAt,end])*nightRateForCar(car);fare=base+extra*extraHours+nightCharge;route=text(data.pickupLocation,300);data={...data,nightCharge};}
  ...
  if(!['one-way','round-trip'].includes(journeyType))fail('Choose one-way or round trip.');
  const routeQuote=await computeOutstationRoute(data.pickupPlaceId,stops,data.destinationPlaceId,journeyType);
  const days=Math.max(1,istDayOrdinal(data.returnAt)-istDayOrdinal(data.pickupAt)+1);
  const includedKm=Math.max(routeQuote.routeKmRoundedUp,days*rules.minimumOutstationKmPerDay);
  const nightCharge=nightCount([data.pickupAt,data.returnAt])*nightRateForCar(car);
  fare=includedKm*number(car.outstation_rate_per_km)+days*OUTSTATION_DRIVER_ALLOWANCE_PER_DAY+nightCharge;
  route=[OUTSTATION_BASE_ADDRESS,pickupLocation,...stops.map(s=>s.name),destination,OUTSTATION_BASE_ADDRESS].join(' → ');
  data={...data,pickupLocation,destination,stops,journeyType,days,googleRouteKm:routeQuote.distanceKmExact,includedKm,driverAllowancePerDay:OUTSTATION_DRIVER_ALLOWANCE_PER_DAY,nightCharge,routeLegs:routeQuote.legs};
 }
