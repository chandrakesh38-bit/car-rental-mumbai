...
export async function validateBookingData(serviceMode,data){
 ...
 else if(tripType==='airport'){if(!AIRPORTS.has(data.airportTerminal)||!['pickup','drop'].includes(data.airportType))fail('Invalid airport transfer.');if(data.airportType==='drop')await validatePickupServiceArea(data.pickupPlaceId);const nightCharge=nightCount([data.pickupAt])*nightRateForCar(car);fare=number(car['airport_'+data.airportTerminal+'_rate'])+nightCharge;route=text(data.pickupLocation,300);data={...data,nightCharge};}
 else {
