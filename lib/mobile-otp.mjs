...
export async function verifyAccessToken(accessToken, phone, purpose, origin) {
  ...
  // MSG91's successful verifyAccessToken response contains only message/type.
  // Inspect only JWT claim names (and a masked identity candidate) server-side
  // so we can bind the provider-verified token to the submitted mobile safely.
  try {
    const parts = accessToken.split('.');
    if (parts.length === 3) {
      const jwtPayload = JSON.parse(new TextDecoder().decode(decode(parts[1])));
      const jwtIdentity =
        jwtPayload.mobile ?? jwtPayload.identifier ?? jwtPayload.phone ?? jwtPayload.number ??
        jwtPayload.sub ?? jwtPayload.data?.mobile ?? jwtPayload.data?.identifier ??
        jwtPayload.data?.phone ?? jwtPayload.data?.number;
      console.info('[mobile-otp] MSG91 access-token claims', {
        claimFields: Object.keys(jwtPayload || {}),
        dataFields: jwtPayload?.data && typeof jwtPayload.data === 'object' ? Object.keys(jwtPayload.data) : [],
        maskedIdentity: maskIdentity(jwtIdentity)
      });
    } else {
      console.info('[mobile-otp] MSG91 access-token claims', { format: 'opaque' });
    }
  } catch {
    console.info('[mobile-otp] MSG91 access-token claims', { format: 'unreadable' });
  }
