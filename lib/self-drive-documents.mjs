...
export async function db(table, query = '', options = {}) {
  ...
  if (!response.ok) {
    // Temporary server-only diagnostics. Do not log request bodies, auth headers,
    // service-role keys, tokens, or customer data.
    let providerError = {};
    try { providerError = await response.clone().json(); } catch {}
    console.info('[self-drive] Supabase DB error', {
      table,
      ...
      hint: typeof providerError?.hint === 'string' ? providerError.hint : undefined
    });
    fail('Unable to save document verification. Please try again.', 503);
  }
