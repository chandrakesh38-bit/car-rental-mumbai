...
export async function db(table, query = '', options = {}) {
  ...
  if (!response.ok) {
    let message = 'Unable to save document verification.';
    try {
      const error = await response.clone().json();
      if (error?.code) message += ' [' + String(error.code).slice(0, 20) + ']';
      if (error?.message) message += ' ' + String(error.message).slice(0, 220);
    } catch {}
    fail(message + ' Please try again.', 503);
  }
