...
try {
 ...
 for(const mode of ['local','outstation','airport','selfdrive']) {
  ...
  const message=await page.locator('#success-confirmation-modal h3 + p').innerText();if(!admin)assert.match(message,/team email could not be sent/);else if(sent)assert.match(message,/acknowledgement has been sent/);else assert.match(message,/could not be sent/);
  assert.match(await page.locator('#success-application-number').innerText(),/^CWD-WD-/);await page.evaluate(()=>closeSuccessModal());
 }
 await page.evaluate(()=>showSuccessModal('TEST-PARTNER',{customer_email_sent:false}));assert.doesNotMatch(await page.locator('#success-confirmation-modal h3 + p').innerText(),/has been sent/);
